  ...
  const ImageModal = ({ imageUrl, onClose }) => (
    <div className="image-modal-overlay" onClick={onClose}>
      <div className="image-modal-content">
        ...
        <img src={imageUrl} alt="Recipe" onClick={(e) => e.stopPropagation()} />
      </div>
    </div>
  );
        ...
        <div className="action-buttons-card">
          {mode === 'view' ? (
            ...
            >
              ...
              Return to Recipe Manager
            </button>
          ) : (
            <>
              <button 
                type="submit" 
                className={`create-btn ${isSubmitting ? 'loading' : ''}`}
                disabled={isSubmitting}
              >
                {isSubmitting ? (
                  <>
                    <svg className="spinner" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <path d="M12 2v4m0 12v4M4 2h16"/>
                    </svg>
                  ...
                  <>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      {mode === 'edit' ? (
                        <path d="M20 6L9 17l-5-5M4 20h16"/>
                      ) : (
